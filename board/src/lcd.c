...
{ 	
  ...
  FSMC_NORSRAMTimingInitTypeDef  readWriteTiming; 
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOE|RCC_AHB1Periph_GPIOB, ENABLE);//使能PD,PE,PB时钟  
  RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FSMC,ENABLE);//使能FSMC时钟  
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;        //PB10 推挽输出,控制背光
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;     //普通输出模式
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;    //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;      //上拉
  GPIO_Init(GPIOB, &GPIO_InitStructure);            //初始化 //PB10 推挽输出,控制背光
	////////////
	/////////////////
	///////////16位数据位和WE OE/////////////
	{
...
/****************************************************************************
* 名    称: void LCD_Draw_Picture(u16 sx,u16 sy,u16 ex,u16 ey,u8 *color)
* 功    能：在指定区域内画入图片
...
****************************************************************************/
///////8位数据的字库
void LCD_Draw_Picture_8(u16 sx,u16 sy,u16 ex,u16 ey,const u8 *color)
{  
	u16 height,width;
	u16 i,j;
	u32 k = 0;
	...
	for(i=0;i<height;i++)
	{
		...
		for(j=0;j<width;j++)
		{
			LCD->LCD_RAM=(color[k+1]<<8)|(color[k]);//写入颜色值
			k++;
			k++;
		}
	}	  
